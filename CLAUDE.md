...
9. **Cookie-consent banner intercepts clicks (Sept 2026)**: Securus added a cookie banner that appears in EVERY fresh browser session (cron browsers carry no cookies). It intercepted the Send interaction — sends failed with "No confirmation modal appeared after clicking Send" — and slowed page loads (navigation timeouts). Handled by `acceptCookieBanner` in auth.mjs (clicks Accept/Allow Cookies, removes lingering consent containers), called at login and twice in the compose flow. Diagnose site changes with `/compose-recon` (read-only: sent-folder rows + compose structure vs selectors, with button visibility).
10. **INSUFFICIENT STAMPS modal**: when stamps run out, clicking Send opens a modal with "CANCEL / PURCHASE STAMPS" instead of Confirm — looks identical to a missing-Confirm failure. compose.mjs detects it and returns `insufficientStamps: true`; phaseSend leaves parts `pending` (auto-resume next cron after stamps purchased) and SMSes Dennis once per 24h. Stamp purchasing is being AUTOMATED (direction change 2026-08-18): recon of the purchase flow runs in phaseScan (`stamp_purchase_recon` state); the autobuy engine (`src/securus/stamps.mjs`, `/stamp-autobuy` endpoint) is guarded — disabled by default, low-water trigger, daily/weekly caps, every attempt logged + SMSed. The purchase click-path stays inert until recon-verified selectors are reviewed; never best-guess selectors in a purchase flow.
